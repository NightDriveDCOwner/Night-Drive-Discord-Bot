...
class VoiceLogging(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is None and after.channel is not None:
            ...
            embed = discord.Embed(title=f"User entered voice channel <#{after.channel.id}>!", color=0x4169E1)                                                                
        ...
        # Überprüfen, ob der Benutzer einen Voice-Channel verlässt
            ...
            embed = discord.Embed(title=f"User leaved voice channel <#{before.channel.id}>!", color=0xFF0000)                                                                
            embed.set_author(name=member.name, icon_url=avatar_url)                          
            embed.set_footer(text=f"ID: {member.id} - heute um {(current_datetime + timedelta(hours=1)).strftime('%H:%M:%S')} Uhr")           
        ...
        # Überprüfen, ob der Benutzer seine Lautstärke ändert
            ...
            if after.deaf:
                ...
                embed = discord.Embed(title=f"User was generally muted in <#{after.channel.id}>!", color=0xFFA500)                                                                
                embed.set_author(name=member.name, icon_url=avatar_url)                          
                embed.set_footer(text=f"ID: {member.id} - heute um {(current_datetime + timedelta(hours=1)).strftime('%H:%M:%S')} Uhr")                
            elif after.mute:
                print(f"{member.name} microphone was muted.")
                embed = discord.Embed(title=f"Users microphone was muted in <#{after.channel.id}>!", color=0xFFA500)                                                                
                embed.set_author(name=member.name, icon_url=avatar_url)                          
                embed.set_footer(text=f"ID: {member.id} - heute um {(current_datetime + timedelta(hours=1)).strftime('%H:%M:%S')} Uhr")                   
            else:
                print(f"{member.name} no longer muted.")
                embed = discord.Embed(title=f"User no longer muted <#{after.channel.id}>!", color=0x006400)                                                                
                embed.set_author(name=member.name, icon_url=avatar_url)                          
                embed.set_footer(text=f"ID: {member.id} - heute um {(current_datetime + timedelta(hours=1)).strftime('%H:%M:%S')} Uhr")   
            ...
            embed = discord.Embed(title=f"User leaved voice channel <#{before.channel.id}> and entered voice channel <#{after.channel.id}>!", color=0x4169E1)                                                                
            embed.set_author(name=member.name, icon_url=avatar_url)                          
            embed.set_footer(text=f"ID: {member.id} - heute um {(current_datetime + timedelta(hours=1)).strftime('%H:%M:%S')} Uhr")                                 
